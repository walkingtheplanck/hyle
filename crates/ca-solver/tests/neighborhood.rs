...
fn moore_center_is_stored() {
    let n = filled_moore(cell(42));
    assert_eq!(n.center(), cell(42));
}

#[test]
fn moore_pos_is_stored() {
    let n = filled_moore(cell(0));
...
fn moore_get_returns_sampled_values() {
    let n = filled_moore(cell(0));
    assert_eq!(n.get(-1, -1, -1), cell(0));
    assert_eq!(n.get(1, 1, 1), cell(26));
}
...
fn moore_count_with_predicate() {
    let mut n = runtime_neighborhood(NeighborhoodShape::Moore, 1, NeighborhoodFalloff::Uniform);
    n.fill(cell(0), [0, 0, 0], |dx, dy, dz| {
        cell((dx + dy + dz + 3) as u32)
    });
    let count = n.count(|e| e.cell.0 > 4);
    assert!(count > 0);
    assert!(count < 26);
}
...
fn moore_symmetry_opposite_offsets_are_different() {
    let mut n = runtime_neighborhood(NeighborhoodShape::Moore, 1, NeighborhoodFalloff::Uniform);
    n.fill(cell(0), [0, 0, 0], |dx, dy, dz| {
        cell(((dx + 2) * 100 + (dy + 2) * 10 + (dz + 2)) as u32)
    });
...
fn vn_r1_only_face_adjacent() {
    ...
    n.fill(cell(0), [0, 0, 0], |_, _, _| cell(1));
    assert_eq!(n.get(1, 0, 0), cell(1));
    assert_eq!(n.get(-1, 0, 0), cell(1));
    assert_eq!(n.get(0, 1, 0), cell(1));
    assert_eq!(n.get(0, -1, 0), cell(1));
    assert_eq!(n.get(0, 0, 1), cell(1));
    assert_eq!(n.get(0, 0, -1), cell(1));
}
...
fn spherical_r2_includes_face_and_edge_but_not_corner() {
    ...
    n.fill(cell(0), [0, 0, 0], |_, _, _| cell(1));
    assert_eq!(n.get(2, 0, 0), cell(1));
    assert_eq!(n.get(1, 1, 0), cell(1));
    assert_eq!(n.get(1, 1, 1), cell(1));
}
...
fn iter_entries_have_correct_offsets() {
    ...
    n.fill(cell(0), [0, 0, 0], |dx: i32, dy: i32, dz: i32| {
        cell((dx.abs() + dy.abs() + dz.abs()) as u32)
    });
    for entry in n.iter() {
        let manhattan = entry.offset.dx.abs() + entry.offset.dy.abs() + entry.offset.dz.abs();
        assert_eq!(manhattan, 1);
        assert_eq!(entry.cell, cell(1));
    }
}
